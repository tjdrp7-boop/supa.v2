...
(() => {
  ...
  segBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      toast((typeInput.value === "solution") ? "솔루션 데모 신청으로 설정했습니다." : "대행 상담 신청으로 설정했습니다.");
    });
  });
...
function setCalcRecommendation({ cpa }) {
  ...
  const rec = isSolution
    ? "현재 계약 CPA가 높습니다. 솔루션 데모에서 데이터 연결과 개선 포인트를 빠르게 점검해보세요."
    : "현재 계약 CPA 기준으로 최적화 여지가 있습니다. 무료 진단에서 개선 우선순위를 안내해드립니다.";
...
window.calculateCPA = function(){
  ...
  if (!adCost || !contracts) {
    resultEl.textContent = "월 광고비와 월 계약 수를 입력해 계약 CPA를 확인해보세요.";
    toast("값을 입력해주세요.");
...
window.resetCalculator = function(){
  ...
  if (resultEl) resultEl.textContent = "값을 입력하면 현재 계약 CPA(월 기준)가 표시됩니다.";
  if (tipsEl) tipsEl.hidden = false;
  if (actionsEl) actionsEl.hidden = true;
...
(() => {
  ...
  form.addEventListener("submit", (e) => {
    ...
    setTimeout(() => {
      ...
      toast("신청이 접수되었습니다. 담당자가 곧 연락드립니다.");
      form.reset();
